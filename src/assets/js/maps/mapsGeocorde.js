...
function xz() {
  let lati = '';
  let longi = '';
  let getLat = localStorage.getItem('latbox');
  getLat = isNaN(getLat);
  if (getLat == true) {
    lati = 4.710989;
    longi = -74.072092;
  } else {
    lati = localStorage.getItem('latbox');
    longi = localStorage.getItem('lonbox');
  }



  if (lati && longi) {
    ll = new google.maps.LatLng(lati, longi);
    zoom = 17;
  } else {
...
function computepos(point) {
  ...
  localStorage.setItem('latbox', point.lat().toFixed(6));
...
function showLatLong(latitude, longitude) {
  ...
  //  alert(latitude);
  // alert(longitude);
  let lati = localStorage.getItem('latbox');
  let longi = localStorage.getItem('lonbox');
...
function latlongwiki() {
  if (posset == 0) {
    alert("Position Not Set.  Please click on the spot on the map to set the map point.");
    return;
  }
  var point = map.getCenter();
  var llwiki_str = "https://www.wikipalapp.com/?title=Latitude-Longitude%20Point&l=" + point.lat() + "&g=" + point.lng();
...
function savegeocordenates() {

  var latitudeguarda = $('#latbox').val();
  var longitudguarda = $('#lonbox').val();

  //var dataString = 'latitudeguarda=' + latitudeguarda + '&longitudguarda=' + longitudguarda;
  alert(latitudeguarda);
  alert(longitudguarda);

  $.ajax({

    data: 'latitudeguarda=' + latitudeguarda + '&longitudguarda=' + longitudguarda,
    type: 'POST',
    url: '../php/insertgeocordenates.php'
  }).done(function (resp) {
    alert('se realiza la inserccion de cordenadas en la Bd');
    //lista_libros('');
...
function selectcordenates() {   //////////////////////////////////////////////////////////////////////////aqui select 

  var buscarid = $('#buscarid').val();
  //var dataString = 'latitudeguarda=' + latitudeguarda + '&longitudguarda=' + longitudguarda;
  alert(buscarid);
  $.ajax({
    data: 'buscarid=' + buscarid,
    type: 'POST',
    url: '../php/selectgeocordenates.php'
  }).done(function (resp) {
    ...
    for (i = 0; i < valores.length; i++) {
      ...
      ///estos son los valores que salen para hacer el cambio
      datos = valores[i][0] + "*" + valores[i][1];
      html += "<tr><td><button class='btn btn-warning' data-toggle='modal' data-target='#modallibros' onclick='mostrar(" + '"' + datos + '"' + ");'><span class='glyphicon glyphicon-pencil'></span></button></td></tr>";
