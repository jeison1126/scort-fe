...
export class ShopdetailsComponent implements OnInit, OnChanges {
  ...
  title = 'gmaps';
   ...
   public getImagePath(imgPath: string, index: number) {
    const ba = document.querySelector('.border-active')
    const img = document.getElementById(index + '_img')
    if(ba){
      ba.classList.remove('border-active');
      this.mainImgPath = imgPath;
      if(img){
        img.className += " border-active";
      }
    }
   }
   ...
   public addToCart(value: any) {
      ...
      if (value.valid_quant == 0) {
         ...
         let products1 = localStorage.getItem("cart_item")
         if(products1){
          let products = JSON.parse(products1) || [];
